(ns realworld-clojure.integration.list-articles
  (:require
   ...
   [realworld-clojure.integration.common :refer [list-articles-request multiple-no-auth-article-schema multiple-auth-article-schema articles-match-feed? get-login-token]]
...
(deftest filter-by-author-auth
  (test-utils/with-system
    ...
    (let [db (get-in sut [:database :datasource])
          ...
          article-one (test-utils/create-article db (:id author-one))
          article-two (test-utils/create-article db (:id author-two))
          r (list-articles-request (str "?author=" (:username author-one)) (get-login-token author-one))]
...
(deftest filter-by-favorited-auth
  (test-utils/with-system
    ...
    (let [db (get-in sut [:database :datasource])
          ...
          _ (test-utils/fav-article db user-two a-one-two)
          _ (test-utils/fav-article db user-two a-two-one)
          r (list-articles-request (str "?favorited=" (:username user-one)) (get-login-token user-two))]
      (validate-response r [a-one-one a-two-two] [author-one author-two] [false false]))))
...
(deftest followed-users
  (test-utils/with-system
    ...
    (let [db (get-in sut [:database :datasource])
          ...
          now (jt/local-date-time)
          a1 (test-utils/create-article db (:id author-two) {:createdat (jt/- now (jt/days 2))})
          a2 (test-utils/create-article db (:id author-one) {:createdat (jt/- now (jt/days 1))})
          a3 (test-utils/create-article db (:id author-two) {:createdat now})
